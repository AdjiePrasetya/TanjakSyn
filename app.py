...
# 🔥 IMPORTANT: init DB saat startup (untuk Railway)
init_db()
    ...
    try:
        ...
        if not tanjak:
            tanjak = get_tanjak_by_id("tanjak_lipatan") or {
                "id": "tanjak_lipatan",
                "name": "Tanjak Lipatan",
                "file": "tanjak_lipatan.png",
                "scale": 2.6,
                "v_offset": 0.35,
                "philosophy": "Simbol Keberanian dan Kepemimpinan",
                "price": 50000,
                "artisan": "Pak Hamdan",
                "artisan_wa": "6281234567890",
            }
...
def api_gallery():
    return jsonify({"success": True, "data": get_gallery(limit=20)})


@app.route("/api/gallery/save", methods=["POST"])
def api_gallery_save():
    data = request.get_json(force=True)
    if data.get("tryon_id"):
        save_gallery(data["tryon_id"], data.get("image_path", ""), data.get("tanjak_id", ""))
